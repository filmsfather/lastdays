...
export default function TimeslotManagement() {
  ...
  const fetchTeachers = async () => {
    try {
      const response = await fetch('/api/admin/accounts?role=teacher')
      const data = await response.json()
      if (data.success) {
        setTeachers(data.accounts)
      }
    } catch (error) {
      console.error('교사 목록 조회 실패:', error)
      toast.error('교사 목록 조회에 실패했습니다.')
    }
  }
  ...
  const createQuickSlots = async (date: string, sessionPeriod: 'AM' | 'PM') => {
    ...
    try {
      const response = await fetch('/api/admin/slots/create-timeslots', {
        method: 'POST',
        headers: {
  ...
  // 개별 슬롯 생성
  const createSingleSlot = async (date: string, timeSlot: string, sessionPeriod: 'AM' | 'PM') => {
    if (!selectedTeacher) {
      toast.error('교사를 먼저 선택해주세요.')
      return
    }
    ...
    try {
      const response = await fetch('/api/admin/slots/create-single-slot', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const toggleBreakTime = async (slot: TimeSlot) => {
    ...
    try {
      const response = await fetch('/api/admin/slots/manage-timeslots', {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // 주간 타임슬롯 조회
  const fetchWeekSlots = async (teacherId: string) => {
    if (!teacherId) {
      setTimeSlots([])
      return
    }
  ...
  const groupedSlots = timeSlots.reduce((acc, slot) => {
    const key = slot.session_period
    if (!acc[key]) acc[key] = []
    acc[key].push(slot)
    return acc
  }, {} as Record<'AM' | 'PM', TimeSlot[]>)
  ...
  return (
    <div className="min-h-screen gradient-bg">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-lg font-bold text-gray-900">Last Days Admin</span>
                <p className="text-sm text-gray-600">타임슬롯 관리</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">타임슬롯 관리</h1>
          <p className="text-xl text-gray-600">10분 단위 예약 타임슬롯을 생성하고 관리하세요</p>
        </div>
